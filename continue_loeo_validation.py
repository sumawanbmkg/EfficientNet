#!/usr/bin/env python3
"""
Continue LOEO Validation from fold 8
Completes remaining folds 8, 9, 10
"""
...
def create_event_based_folds(metadata_path, n_folds=10, random_state=42):
    ...
    event_info = df.groupby('event_id').agg({'magnitude_class': 'first'}).reset_index()
    event_to_indices = {eid: df[df['event_id'] == eid].index.tolist() for eid in event_info['event_id']}
    
    skf = StratifiedKFold(n_splits=n_folds, shuffle=True, random_state=random_state)
    folds = []
    for fold_idx, (train_event_idx, test_event_idx) in enumerate(skf.split(event_info['event_id'], event_info['magnitude_class'])):
        ...
        for eid in train_events:
            train_indices.extend(event_to_indices[eid])
        for eid in test_events:
            test_indices.extend(event_to_indices[eid])
        folds.append({
            'fold': fold_idx + 1,
...
def train_fold(fold_info, full_dataset, config, device):
    ...
    model = EfficientNetMultiTask(num_magnitude_classes=config['num_magnitude_classes'], num_azimuth_classes=config['num_azimuth_classes']).to(device)
    ...
    for epoch in range(config['epochs']):
        ...
        print(f"  Epoch {epoch+1}: Train Mag={train_results['mag_acc']:.1f}%, Azi={train_results['azi_acc']:.1f}% | Test Mag={test_results['mag_acc']:.1f}%, Azi={test_results['azi_acc']:.1f}%")
        if combined_acc > best_acc:
            ...
            if patience_counter >= config['patience']:
                ...
                break
    return {'fold': fold_info['fold'], 'magnitude_accuracy': best_results['mag_acc'], 'azimuth_accuracy': best_results['azi_acc'], 'n_train_events': fold_info['n_train_events'], 'n_test_events': fold_info['n_test_events']}


def main():
    print("\n" + "="*60)
    print("CONTINUING LOEO VALIDATION - Folds 8, 9, 10")
    print("="*60)
    ...
    existing_results = []
    for i in range(1, 8):
        fold_file = output_dir / f'loeo_fold_{i}.json'
        if fold_file.exists():
    ...
    # Train remaining folds (8, 9, 10)
    new_results = []
    for fold in folds[7:]:  # Folds 8, 9, 10 (index 7, 8, 9)
    ...
    final_results = {
        ...
        'magnitude_accuracy': {'mean': float(np.mean(mag_accs)), 'std': float(np.std(mag_accs)), 'min': float(np.min(mag_accs)), 'max': float(np.max(mag_accs))},
        'azimuth_accuracy': {'mean': float(np.mean(azi_accs)), 'std': float(np.std(azi_accs)), 'min': float(np.min(azi_accs)), 'max': float(np.max(azi_accs))},
    ...
    # Print summary
    print("\n" + "="*60)
    print("COMPLETE LOEO VALIDATION RESULTS (10 Folds)")
    print("="*60)
    ...
    random_mag, random_azi = 94.37, 57.39
    ...
    if mag_drop < 0 and azi_drop < 0:
        print(f"\n✅ EXCELLENT! LOEO results are BETTER than random split!")
        print("   This indicates NO overfitting - model generalizes well to unseen events!")
    elif mag_drop < 5 and azi_drop < 5:
        print(f"\n✅ Good generalization! Drop is within acceptable range (<5%)")
    else:
