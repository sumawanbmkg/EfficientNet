"""
Generate Medium Earthquake Dataset (M5.0 - M5.9) - FIXED VERSION
==================================================================
Script ini adalah kloning dari generate_dataset_moderate.py tapi ditujukan untuk
scanning data kelas MEDIUM (M5.x) agar dataset lebih seimbang dengan Large (M6+).

Fitur:
- Filter M5.0 - M5.9
- Tahun 2023-2025 (Senada dengan Moderate/Large)
- Target: 300-400 Samples
- Menggunakan parameter scan & koordinat yang sama.
"""
...
STATION_FILE = 'mdata2/lokasi_stasiun.csv'
OUTPUT_DIR = 'dataset_medium_new' # Folder Baru untuk Medium
TARGET_COUNT = 500
...
class MediumScanner:
    ...
    def find_nearest_station(self, eq_lat, eq_lon):
        ...
        # Threshold: 600 km untuk Medium (Sinyal M5.x lebih kuat dari M4.x, jadi radius lebih besar)
        if min_dist < 600:
    ...
    def process_event(self, event):
        ...
        # Output Filename (Force Magnitude Class Medium)
    ...
    def _create_metadata(self, event, station, date, hour, filename):
        return {
            ...
            'magnitude_class': 'Medium', # FORCE LABEL
            'azimuth': 0, 
            'azimuth_class': 'Unknown'
        }

def main():
    logger.info("PHASE 2 - MEDIUM DATASET GENERATOR (M5.x)")
    
    df = pd.read_csv(REPO_FILE)
    df['dt'] = pd.to_datetime(df['datetime'])
    
    # Filter (Priority 2023-2025 first, then older)
    df_new = df[df['dt'].dt.year >= 2023]
    df_old = df[df['dt'].dt.year < 2023]
    
    # Target M5.0 - M5.9
    df_new = df_new[(df_new['Magnitude'] >= 5.0) & (df_new['Magnitude'] < 6.0)]
    df_old = df_old[(df_old['Magnitude'] >= 5.0) & (df_old['Magnitude'] < 6.0)]
    
    df = pd.concat([df_new, df_old])
    
    logger.info(f"Candidates (M5.0-5.9, >=2023): {len(df)}")
    
    # If there are many candidates, shuffle but don't limit too strictly
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Scanning Medium"):
