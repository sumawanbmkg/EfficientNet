#!/usr/bin/env python3
"""
Finetune ConvNeXt Training Script (Server/GPU Optimized)
Finetunes the best checkpoint with fixed image size at 224x224
Designed for High-Performance GPU Servers
"""
...
class FinetuneTrainer(ConvNeXtTrainer):
    ...
    def __init__(self, checkpoint_path, config_override=None):
        ...
        self.exp_dir = Path(f"experiments_convnext/finetune_v3_gpu_{timestamp}")
    ...
    def finetune(self):
        ...
        finetune_lr = self.config.get('base_lr', 1e-4)
        ...
        # New scheduler with Cosine Annealing
        ...
        # Note: setup_data in base class uses config['batch_size'] and defaults num_workers=4
        # We should ensure config has updated batch_size if overridden
...
def main():
    parser = argparse.ArgumentParser(description='Finetune ConvNeXt Training (GPU Optimized)')
    parser.add_argument('--checkpoint', type=str, required=True,
                       help='Path to BEST checkpoint file (checkpoint_best.pth)')
    parser.add_argument('--epochs', type=int, default=50,
                       help='Number of finetuning epochs')
    parser.add_argument('--batch-size', type=int, default=32,
                       help='Batch size (increase for powerful GPUs)')
    parser.add_argument('--workers', type=int, default=8,
                       help='Number of data loader workers')
    
    args = parser.parse_args()
    
    # Override config for GPU Server Optimization
    config_override = {
        'epochs': args.epochs,
        'resize_schedule': {0: 224},  # Force 224x224 from start
        'base_lr': 1e-4,              # Reduced LR for finetuning
        'batch_size': args.batch_size, # Optimized batch size
        'num_workers': args.workers    # Optimized workers count
    }
